...
exports.normalizeWatchStatus = normalizeWatchStatus;
exports.parseStreamingProviders = parseStreamingProviders;
 ...
 * ```
 */
function normalizeWatchStatus(status) {
    if (!status) {
        return 'not_watched';
    }
 ...
 * ```
 */
function parseStreamingProviders(providers) {
    if (!providers) {
        return [];
    }
    try {
 ...
 * ```
 */
function calculateMatchConfidence(candidateTitle, candidateYear, originalTitle, originalYear, hasPoster = false) {
    let score = 0;
 ...
 * ```
 */
function normalizeRating(rating, scale = 10) {
